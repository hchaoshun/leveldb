...
bool Reader::ReadRecord(Slice* record, std::string* scratch) {
  ...
  //进入到while(true)循环，直到读取到KLastType或者KFullType的record，或者到了文件结尾。
  ...
  while (true) {
    ...
    //存储当前正在读取的record的偏移值。接下来根据不同的record_type类型，分别处理，一共有7种情况
    ...
    switch (record_type) {
      //完整的log record，成功返回读取的user record数据
      case kFullType:
      ...
      //说明是一系列log record(fragment)中的最后一条
      case kLastType:
        if (!in_fragmented_record) {
          ReportCorruption(fragment.size(),
...
//读取到一个完整的record。内容存放在成员变量buffer_中
  ...
  while (true) {
    if (buffer_.size() < kHeaderSize) {
      if (!eof_) {
        ...
        // just report EOF.
        //如果eof_为true并且buffer为空，表明已经到了文件结尾，正常结束，返回kEof。
        buffer_.clear();
        return kEof;
      }
    }
