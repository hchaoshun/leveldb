...
//将原 src 中的属性进行了安全裁剪，
//并且创建了 dbname 目录、将原 Info Log 文件重命名并创建了新的 Info Log 文件，也就是 testdb/LOG 文件
...
//NewDB 中创建一个新的 VersionEdit 对象，将日志编号设为 0，MANIFEST 编号设为 1，NextFile 编号自然是 2 了。
//而后将 VersionEdit 对象转为日志 Record 写入 MANIFEST 文件中，并将 CURRENT 指向该 MANIFEST。
Status DBImpl::NewDB() {
  VersionEdit new_db;
...
Status DBImpl::Recover(VersionEdit* edit, bool* save_manifest) {
  ...
  //对于新创建的DB而言，CURRENT文件肯定是不存在的。那么就需要创建
  if (!env_->FileExists(CurrentFileName(dbname_))) {
    // 如果不存在的时候，需要创建DB，即写manifest文件
  ...
  //manifest文件编号设置为2，故最终输出的manifest为2
  //save_manifest设置为true
  //current->manifest->versionedit->version->versionset
  ...
  // 000008.ldb      000014.ldb      000020.ldb      CURRENT         LOG
  std::vector<std::string> filenames;
  ...
  for (size_t i = 0; i < filenames.size(); i++) {
    ...
    if (ParseFileName(filenames[i], &number, &type)) {
      ...
      //todo 为什么要number >= min_log
      if (type == kLogFile && ((number >= min_log) || (number == prev_log)))
        logs.push_back(number);
    }
  }
  // 主要是为了保证filename和expected里面的数字是一一对应的！！
  if (!expected.empty()) {
    char buf[50];
...
Status DBImpl::RecoverLogFile(uint64_t log_number, bool last_log,
  ...
  // Open the log file
  // 利用给定的log_number，生成相应的xxxx.log文件名。然后打开之。
...
//将memtable写入sst。具体是哪一层需要由[smallest, largest]决定
//base == nullptr 表示写入level0
...
//minor compaction 过程
//每次compaction结束后，都会生成一个新的sst文件，也意味着leveldb的版本状态发生了变化，会进行
//一个版本的更替
...
void DBImpl::BackgroundCompaction() {
  ...
  //设置manual_compaction_ 的时候被执行，它主要是用于测试数据库的compaction功能，用于debug
  if (is_manual) {
...
Status DBImpl::Write(const WriteOptions& options, WriteBatch* updates) {
  ...
  //检查内存中的Memtable是否有足够的空间可供写
  Status status = MakeRoomForWrite(updates == nullptr);
  uint64_t last_sequence = versions_->LastSequence();
  Writer* last_writer = &w;
  if (status.ok() && updates != nullptr) {  // nullptr batch is for compactions
    //遍历当前writers_中的所有Writer，并将他们组合。
  ...
  //while循环就是将已经处理的任务从队列中移除的过程，
  //同时还会通知相应任务的生产者线程说明它所添加的任务已经处理完毕了(通过设置writer.done标记位)，可以直接返回了
  while (true) {
...
WriteBatch* DBImpl::BuildBatchGroup(Writer** last_writer) {
  ...
  //设置每次批量写入的最大的数据量，防止一次插入过多数据导致等待写完成的时间过长
...
Status DBImpl::MakeRoomForWrite(bool force) {
  ...
  while (true) {
    if (!bg_error_.ok()) {
      ...
      s = bg_error_;
      break;
    //  如果发现level 0 中的文件过多，则说明需要背景线程进行合并，
    //  因此需要等待背景线程合并完成之后再写入，所以延迟了写。
      ...
      mutex_.Lock();
    //while循环只有在这里才会正确地返回，也就是说，这个while循环会一直继续下去直到发生错误或者mem_有足够空间可写
    } else if (!force &&
               (mem_->ApproximateMemoryUsage() <= options_.write_buffer_size)) {
      // There is room in current memtable
      break;
    //  当执行到这里，说明mem_没有足够的空间可供写，此时试图将mem_赋值给imm_，
    //  然后重新分配一个mem_供用户写，但在此之前必须先检查imm_是否为空，如果它不为空的话，
    //  说明上次赋值给imm_的mem还没有被背景线程写入磁盘，那只能等待了，不然就会覆盖掉之前的mem_。
    } else if (imm_ != nullptr) {
      // We have filled up the current memtable, but the previous
      // one is still being compacted, so we wait.
      Log(options_.info_log, "Current memtable full; waiting...\n");
      background_work_finished_signal_.Wait();
    //因为之前将imm_写盘完成后，level 0 中的sstable又增加了一个文件，
    //我们必须判断此时level 0 中的文件数量是否太多，太多的话那就还得等一会
    } else if (versions_->NumLevelFiles(0) >= config::kL0_StopWritesTrigger) {
      // There are too many level-0 files.
      Log(options_.info_log, "Too many L0 files; waiting...\n");
      background_work_finished_signal_.Wait();
    //条件的都满足了，把mem_复制给imm_，并为mem_重新分配一个新的Memtable
      ...
      // 有可能启动后台compaction线程的地方（因为可能后台compactiong线程已经启动了）,
      // 后台只对imm_处理，不会对mem_处理
      MaybeScheduleCompaction();
    }
  }
  return s;
